...
describe('createCameraAnimation', () => {
  it('creates an active animation', () => {
    const from: Camera = { centerRa: 0, centerDec: 0, zoom: 400 };
    const to: Camera = { centerRa: 12, centerDec: 45, zoom: 600 };
...
describe('tickCameraAnimation', () => {
  ...
  it('interpolates camera over time', () => {
    const from: Camera = { centerRa: 2, centerDec: 0, zoom: 400 };
    const to: Camera = { centerRa: 8, centerDec: 60, zoom: 800 };
    ...
    const end = tickCameraAnimation(anim, startTime + 1001);
    expect(end!.done).toBe(true);
    expect(end!.cam.centerRa).toBeCloseTo(8);
  });
});
